...
# Tool Usage SOP (嚴格遵守)
1. **探索階段**：如果不確定資料表名稱，必須先調用 `list_tables`；如果不確定資料表結構，必須先調用 `get_table_schema`。
2. **開發階段**：根據 Schema 撰寫 SQL。禁止使用 `SELECT *`，僅選取必要欄位，並盡量用 `SELECT TOP`。
3. **驗證階段**：若執行失敗，分析錯誤訊息（如欄位誤判），修正參數後再次執行。
4. **分析階段**：若工具執行成功且不需要更多資訊，停止調用工具，分析回傳資料並回答使用者問題。
